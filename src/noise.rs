...
byond_fn!(fn fbm3d(x, y, z, seed, freq, octaves, lacunarity, persistence){
    _fbm3d(x, y, z, seed, freq, octaves, lacunarity, persistence).ok()
});
...
// Not entirely 3d, only should get certain Z level of the noise
pub fn _fbm3d(_x:&str, _y:&str, _z:&str, _seed:&str, _freq:&str, _octaves:&str, _lacunarity:&str, _persistence:&str) -> Result<String, bool> {
    let size_x: i32 = _x.parse().unwrap();
    let size_y: i32 = _y.parse().unwrap();
    let level_z: i32 = _z.parse().unwrap();
    let seed: u32 = _seed.parse().unwrap();
    let freq: f64 = _freq.parse().unwrap();
    let octaves: usize = _octaves.parse().unwrap();
    let lacunarity: f64 = _lacunarity.parse().unwrap();
    ...
    for y in 0..size_y{
        for x in 0..size_x{
            let val_at_cords = fbm.get([x as f64, y as f64, level_z as f64]);
            let _ = write!(string, "{},", val_at_cords.to_string());
        }
    }
    Ok(string)
